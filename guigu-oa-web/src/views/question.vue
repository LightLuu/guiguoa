<template>
    <div class="container">
      ...
      <div class="list-wrap" >
        ...
        >
          <van-cell v-for="item in list" :key="item.id" @click="info(item.id)">
            <template slot="default">
              <div class="item-wrap">
                ...
                <!-- <div class="item-block">
                  <p v-for="(value,key) in item.formProps[0]" v-if="key !== '图片'">{{ key }}：<span v-html="value"></span></p>
                </div>
  ...
  export default {
    ...
    methods: {
      ...
      getquestions(){
       // alert("查询问卷")
        console.log(this.pageNo)
        api.getquestions(this.pageNo, this.pageSize).then(response => {
        //  alert("调用完成")
         // alert(response.data.length)
          ...
          for (let i=0;i<response.data.records.length;i++) {
            let item = response.data.records[i]
           // alert(item.formProps)
            if(item.formProps == undefined)
            {continue}
            item.formProps = JSON.parse(item.formProps)
           // alert(item.formProps)
            this.list.push(item);
          }
         //alert(111)
